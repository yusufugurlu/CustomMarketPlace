...
const SearchInput = ({ show, setShow }) => {
  ...
  const { isChangeMenu, menuData } = useSelector((state) => state.menuData);

  const [menus, setMenus] = useState([]);

  const routeList = useMemo(
    () =>
      menuHelper.getMenu(menuData),
    [isLogin, currentUser, menuData]
  );
  ...
  useEffect(() => {
    if (USE_MULTI_LANGUAGE) {
      const menuList = menuHelper.getMenu(menuData);
      const formatedRoutes = menuList.map((d) => ({
        ...d,
        label: d.label || 'menu.home',
      }));
