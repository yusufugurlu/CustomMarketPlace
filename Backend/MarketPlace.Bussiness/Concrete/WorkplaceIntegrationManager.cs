    ...
    public class WorkplaceIntegrationManager : IWorkplaceIntegrationService
    {
        ...
        public async Task<ServiceResult> CreateIntegration(CreateWorkplaceIntegrationDto dto, string lang)
        {
            if (dto != null && dto.WorkPlaceId != 0)
            {
                ...
                if (dto.Id == 0) //added
                {
                    var hasAnySameIntegrationType = await _integrationForWorkPlaceRepository
                        .GetAllToList(x => !x.IsDeleted && x.WorkPlaceId == dto.WorkPlaceId && x.IntegrationType == (IntegrationType)dto.IntegrationType);


                    if (hasAnySameIntegrationType.Any())
                    {
                        return Result.Fail("YouCanChooseOneOfSameTypeOfIntegration".GetAlertResourceValue(lang), 500);
                    }
                ...
                else //updated
                {
                    ...
                    var hasAnySameIntegrationType = await _integrationForWorkPlaceRepository
                    .GetAllToList(x => x.Id != dto.Id && !x.IsDeleted && x.WorkPlaceId == dto.WorkPlaceId && x.IntegrationType == (IntegrationType)dto.IntegrationType);
